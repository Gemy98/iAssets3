...
jQuery.validator.addMethod("enName", function(value, element) {
	if (this.optional(element))
		return true;
	//return new RegExp(/^[a-zA-Z]+( [a-zA-Z]+)*$/).test(value);
	return new RegExp(/^[a-zA-Z\s]+$/).test(value);
}, "Invalid English Name.");

jQuery.validator.addMethod("arName", function(value, element) {
	if (this.optional(element))
		return true;
	//return new RegExp(/^[\u0600-\u06FF]+( [\u0600-\u06FF]+)*$/).test(value);
	//return new RegExp(/[\u0600-\u06FF]/).test(value);
	return new RegExp(/^[\u0600-\u06FF\s]+$/).test(value);
}, "Invalid Arabic Name.");
...
//},"Max file size allowed is {0} bytes.");

//Islam :: to validate select box with the empty value = 0 not ""
jQuery.validator.addMethod('selectrequired', function (value, element, param) {
    return (value != '0');
}, "please select a valid option");

//multiple file size validator Islam
jQuery.validator.addMethod('filesize', function(value, element, param) {
    // param = size (in bytes) 
    // element = element to validate (<input>)
    // value = value of the element (file name)
	...
	for (var i=0; i < list.length ; i++){
    	if(list[i].size > (param*1000000))  // the passed threshold in MB
    		return false;
    } 
    
    return true;
},jQuery.validator.format("Max file size allowed is {0} MB."));

// max attachments number validator
jQuery.validator.addMethod('maxattach', function(value, element, param) {
    // param = size (in bytes) 
    // element = element to validate (<input>)
    // value = value of the element (file name)
	...
	//if(this.optional(element) == true)
	//	return true;
	return (element.files.length) + ($(element).closest('label').parent().find('ul.uploadedFilesList li').length) <= param;
   // return element.files.length <= param;

},jQuery.validator.format("Max attachment number allowed is {0}"));
...
jQuery.validator.addMethod('filerequired', function(value, element, param) {
	...
	//if(this.optional(element) == true)
	//	return true;
	return (element.files.length) + ($(element).closest('label').parent().find('ul.uploadedFilesList li').length) > 0;

},jQuery.validator.format("file upload required"));
...
//		return true;
    ...
    for(var i= 0; i< param.length; i++){
    	$(param[i]).change(function(){$(element).valid()});
    	var oDate = $(param[i]).datepicker('getDate') ;
