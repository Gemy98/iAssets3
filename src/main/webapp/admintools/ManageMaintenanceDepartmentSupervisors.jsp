...
<body>
	<form method="post" action="${processServlet}">
		<table class="layout_grid">
			...
			<tr>
				<td class="side_label_middle"> <fmt:message key="jsp.ManageMaintenanceDepartmentSupervisors.supervisorId" />  :</td>
				<td><select name="supervisorId" id="supervisorId" class="auto_off" onchange='rerenderUI("${displayServlet}",true)'>
						<option value="0">  <fmt:message key="jsp.ManageMaintenanceDepartmentSupervisors.supervisor" /> </option>
				...
				%>
							<option value="<%=supervisor.getId() %>"><%= supervisor.getName(WebUtil.getSessionLanguage(request))%></option>
				<%} %>
				</select></td>
			</tr>
