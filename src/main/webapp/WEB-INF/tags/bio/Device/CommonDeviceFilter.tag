...
<%@ attribute name="classificationList" required="false" type="java.util.List"%>
...
<%@ attribute name="showCategoryAndClassification" required="false" type="java.lang.Boolean"%>
<%@ attribute name="showDepartmentAndLocation" required="false" type="java.lang.Boolean"%>
<%@ attribute name="showDeviceName" required="false" type="java.lang.Boolean"%>
<%@ attribute name="showAgentAndSubcontractor" required="false" type="java.lang.Boolean"%>
<%@ attribute name="showManufacturerNameAndModel" required="false" type="java.lang.Boolean"%>


  <c:if test="${showCategoryAndClassification != null && showCategoryAndClassification == true }">
		<c:if test="${classificationList != null  && !classificationList.isEmpty()}">
			<tr>
				<td class="side_label_middle"><fmt:message key="jsp.CommonDevicesFilter.classification" />  :</td>
				<td> 
				 <select id="classification" name="classification" class="auto_add_all auto_off">
       ...
       <c:if test="${catList != null  && !catList.isEmpty()}">
            <tr>
				<td class="side_label_middle"><fmt:message key="jsp.CommonDevicesFilter.category" /> :</td>
				<td valign="top">
					<span dir="ltr">
						<c:forEach items="${catList }" var="cat">
							<input style="margin-left: 2px; margin-right: 2px;"  name="category"  type="checkbox" value="${ cat.getId() }"/> ${ cat.getLocalizedName() }
						</c:forEach>
					</span>
				</td>
			</tr>
	...
	<c:if test="${showDepartmentAndLocation != null && showDepartmentAndLocation == true }">
       <c:if test="${locationList != null  && !locationList.isEmpty()}">
			<tr>
				<td class="side_label_middle"><fmt:message key="jsp.CommonDevicesFilter.building" />  :</td>
				<td><select id="hospLocation" name="hospLocation" class="auto_add_all auto_off">
		 ...
		 <c:if test="${depList != null  && !depList.isEmpty()}">
			<tr>
				<td class="side_label_middle"><fmt:message key="jsp.CommonDevicesFilter.dep" />  :</td>
	...
	<c:if test="${showDeviceName != null && showDeviceName == true }">
		<tr>
	        <td class="side_label_middle"> <fmt:message key="jsp.CommonDevicesFilter.name" />:</td>
	...
	<c:if test="${showAgentAndSubcontractor != null && showAgentAndSubcontractor == true }">
       	<tr>
	        <td class="side_label_middle"><fmt:message key="jsp.CommonDevicesFilter.agentName" /> :</td>
	        <td><input type="text" size="25" id="agentName" name="agentName"></td>
      	</tr>
      	<tr>
	        <td class="side_label_middle"> <fmt:message key="jsp.CommonDevicesFilter.subcontractor" />  :</td>
	        <td><input type="text" size="25" id="subcontractor" name="subcontractor"></td>
 		</tr>
	</c:if>

	<c:if test="${showManufacturerNameAndModel != null && showManufacturerNameAndModel == true }">
		<tr>
	      <td class="side_label_middle"><fmt:message key="jsp.CommonDevicesFilter.manufacturerName" />:</td>
	      <td><input type="text" size="25" id="manufacturerName" name="manufacturerName"></td>
	    </tr>
	    <tr>
	      <td class="side_label_middle"> <fmt:message key="jsp.CommonDevicesFilter.model" />:</td>
