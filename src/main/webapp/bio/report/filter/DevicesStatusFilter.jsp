...
<body>
	...
	<table class="layout_grid">
       ...
       <tr>
		...
		<tr>
	    	<td class="side_label_middle"> <fmt:message key="jsp.DevicesStatusFilter.deviceStatus" />:</td>
	   		<td> 
	   			  <select id="deviceStatus" name="deviceStatus" class="auto_add_all auto_off">
		        	    <option value="<%= Enums.DEVICE_STATUS.WORK_PROPERLY.getStatus() %>">
							<%=LocalizationManager.getLiteral("jsp.DevicesStatusFilter.workProperly" ,WebUtil.getSessionLanguage(request) )%>
						</option>
		        	    <option value="<%= Enums.DEVICE_STATUS.UNDER_MAINTENANCE.getStatus() %>">
							<%=LocalizationManager.getLiteral("jsp.DevicesStatusFilter.underMaintenance" ,WebUtil.getSessionLanguage(request) )%>
						</option>
		          </select>
		    </td>
	  	</tr>
	   ...
	   <DeviceLib:CommonDeviceFilter catList="${ catList }"
			...
			showAgentAndSubcontractor="false"
		...
		<tr>
			<td>&nbsp;</td>
			<td><input type="submit" value='<fmt:message key="jsp.DevicesStatusFilter.submitBtn" /> '></td>
		</tr>			
	</table>
