...
@WebServlet(value={"/bio/ViewPPMVisitsDetails"})
...
extends BioBasicServlet {
    ...
    public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if ((ppmList = this.bioDBQueryManager.queryJPQL(jpql = jpql + " ORDER BY h.visitDate, h.subcontractor, h.hospitalDevice.category, h.hospitalDevice.name", map, BioHospitalDevicePpmDetail.class)) == null || ppmList.isEmpty()) {
            this.announceErorrMessage(request, response, LocalizationManager.getLiteral("servlet.ViewPPMVisitsDetails.msg", langCode));
            return;
        }
        request.setAttribute("ppmList", ppmList);
        this.viewHTMLReport(request, response, "/bio/report/ViewPPMVisitsDetails.jsp", Enums.REPORT_TITLE.PPM_ATTACHED_FILES.getReportTitle(langCode));
    }
}
