...
@Component(value="bioMaintenanceRequestNotificationsEndPoint")
@ServerEndpoint(value="/bio/MaintenanceRequestsNotifier", configurator=SpringConfigurator.class)
public class MaintenanceRequestNotificationsEndPoint {
    ...
    public void onOpen(Session session) {
        queue.add(session);
        MaintenanceRequestNotificationsEndPoint.sendMessage(session, this.getNotification(WebUtil.getSiteIdFromEndPointSession(session), WebUtil.getStringRequestParameterValueFromEndPointSession(session, "langCode")));
    ...
    private String getNotification(Integer siteId, String langCode) {
        ...
        Long notificationsCount = this.dbQueryManager.getOpenMaintenanceNotifactionsCount(siteId);
        if (notificationsCount == null || notificationsCount == 0L) {
            return " ";
        }
        String notfHtml = "<div id='mreq_notifications_ph'><img src='image/warning.png' />" + LocalizationManager.getLiteral("com.iassets.biomedical.endpoints.MaintenanceRequestNotificationsEndPoint.notification1", langCode) + " (" + notificationsCount + ") .. <a href='bio/ViewMaintenanceRequests?" + "ihf" + "=1' target='_blank' rel='opener'>" + LocalizationManager.getLiteral("com.iassets.biomedical.endpoints.MaintenanceRequestNotificationsEndPoint.notification2", langCode) + "</a><a title='" + LocalizationManager.getLiteral("com.iassets.biomedical.endpoints.MaintenanceRequestNotificationsEndPoint.notification3", langCode) + "' class='close_btn' href='javascript:void(0)' onclick=\"activateNotificationStopperCookie(this,'bio_stop_mreq_notify')\">X</a>";
        return notfHtml + "</div>";
    }
}
