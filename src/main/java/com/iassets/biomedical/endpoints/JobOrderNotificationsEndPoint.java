...
@Component(value="bioJobOrderNotificationsEndPoint")
@ServerEndpoint(value="/bio/JobOrdersNotifier", configurator=SpringConfigurator.class)
public class JobOrderNotificationsEndPoint {
    ...
    public void onOpen(Session session) {
        queue.add(session);
        JobOrderNotificationsEndPoint.sendMessage(session, this.getNotification(WebUtil.getSiteIdFromEndPointSession(session), WebUtil.getStringRequestParameterValueFromEndPointSession(session, "langCode")));
    ...
    private String getNotification(Integer siteId, String langCode) {
        ...
        Long notificationsCount = this.dbQueryManager.getJobOrderNotifactionsCount(siteId);
        if (notificationsCount == null || notificationsCount == 0L) {
            return " ";
        }
        String notfHtml = "<div id='jo_notifications_ph'><img src='image/warning.png' />" + LocalizationManager.getLiteral("com.iassets.biomedical.endpoints.JobOrderNotificationsEndPoint.notification1", langCode) + " (" + notificationsCount + ") .. <a href='bio/ViewLateJobOrders?" + "ihf" + "=1' target='_blank' rel='opener'>" + LocalizationManager.getLiteral("com.iassets.biomedical.endpoints.JobOrderNotificationsEndPoint.notification2", langCode) + "</a><a title='" + LocalizationManager.getLiteral("com.iassets.biomedical.endpoints.JobOrderNotificationsEndPoint.notification3", langCode) + "' class='close_btn' href='javascript:void(0)' onclick=\"activateNotificationStopperCookie(this,'bio_stop_jo_notify')\">X</a>";
        return notfHtml + "</div>";
    }
}
