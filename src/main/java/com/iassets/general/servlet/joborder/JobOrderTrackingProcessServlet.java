...
extends GenBasicServlet {
    ...
    protected void updateNotifications(HttpServletRequest request) {
        ...
        this.joNotifier.broadcastNotificationUpdates(siteId, this.getSessionUser(request).getGmpCategory());
        this.spCategoryNotifier.broadcastNotificationUpdates(siteId);
    }
    ...
    protected void updateNotificationsThenGoForward(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        if (request.getParameter("showJobOrderReportForPrint") != null) {
            this.sendRedirect(request, response, "/gen/ViewJobOrderReport?jobOrderNo=" + request.getParameter("jobOrderNo"));
            return;
        }
        ...
        if (WebUtil.getParamValue(request, "saveThenReturn", null) != null) {
            if (request.getParameter("showJobOrderReport") != null) {
                ...
                this.sendRedirect(request, response, "/gen/FollowupJobOrderSearch");
            }
        } else if (WebUtil.getParamValue(request, "saveThenContinue", null) != null) {
            this.basicForward(request, response, "/gen/FollowupJobOrder");
    ...
    protected void setFinalActionDetails(GenJobOrder jobOrder, HttpServletRequest request) throws FileNotFoundException, IOException, ServletException {
        GenHospitalDevice device = jobOrder.getHospitalDevice();
